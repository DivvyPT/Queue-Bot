    ...
    if (
      ...
      if (hasPermission) {
        ...
        if (parsed.command === cmdConfig.startCmd) {
          ...
          if (parsed.arguments) {
            guild.me
              .setNickname(`(${parsed.arguments}) Satan`)
              .catch(() => null);
          }
          // Color
          ...
          Commands.setServerSetting(
            parsed,
            ["on", "off"].includes(parsed.arguments)
          );
    ...
    if (parsed.command === cmdConfig.displayCmd) {
      ...
      if (args.length >= 1 && args[0] !== "") {
        ...
        MessagingUtils.sendTempMessage(
          "Para entrares na lista de espera tens de escrever !join com os dinos que podes levar para esta raid. Exemplo: !join tryos. Se conseguires fazer com qualquer um escreve !join todos.",
          channel,
          15
        );
      }
  ...
  for (const storedQueueGuild of storedQueueGuilds) {
    try {
      ...
      for (const queueChannel of queueChannels) {
        ...
        const storedQueueMemberIds = (
          await QueueMemberTable.getFromQueue(queueChannel)
        ).map((member) => member.queue_member_id);
        const queueMemberIds = queueChannel.members
          .filter((member) => !Base.isMe(member))
          .keyArray();
...
client.on("channelDelete", async (channel) => {
  ...
  if (deletedQueueChannel) {
    await QueueChannelTable.unstoreQueueChannel(
...
const returningMembersCache = new Map<
  string,
  { member: QueueMember; time: number }
>();
async function markLeavingMember(
  member: GuildMember,
  oldVoiceChannel: VoiceChannel
): Promise<void> {
  // Fetch Member
  const storedQueueMember = await QueueMemberTable.get(
    oldVoiceChannel.id,
    member.id
  );
...
client.on("voiceStateUpdate", async (oldVoiceState, newVoiceState) => {
  ...
  if (
    ...
    (newVoiceChannel &&
      (await MemberPermsTable.isBlacklisted(newVoiceChannel.id, member.id)))
  ) {
  ...
  if (
    Base.isMe(member) &&
    ((storedOldQueueChannel && storedNewQueueChannel) ||
      !oldVoiceChannel ||
      !newVoiceChannel)
  ) {
  ...
  if (storedNewQueueChannel && !Base.isMe(member)) {
    ...
    if (storedNewQueueChannel.target_channel_id) {
      const targetChannel = member.guild.channels.cache.get(
        storedNewQueueChannel.target_channel_id
      ) as VoiceChannel;
      if (targetChannel) {
        if (
          storedNewQueueChannel.auto_fill &&
          newVoiceChannel.members.filter((member) => !member.user.bot).size ===
            1 &&
        ...
        await QueueChannelTable.updateTarget(
          newVoiceChannel.id,
          knex.raw("DEFAULT")
        );
      }
    }
    const returningMember = returningMembersCache.get(
      newVoiceChannel.id + "." + member.id
    );
    returningMembersCache.delete(newVoiceChannel.id + "." + member.id);

    const withinGracePeriod = returningMember
      ? Date.now() - returningMember.time < +queueGuild.grace_period * 1000
      : false;
  ...
  if (storedOldQueueChannel) {
    ...
    if (Base.isMe(member) && newVoiceChannel) {
      ...
      if (blockNextCache.delete(member.id)) {
        ...
        await QueueMemberTable.unstoreQueueMembers(oldVoiceChannel.id, [
          member.id,
        ]);
      } else {
        // Otherwise, cache it
  ...
  if (!Base.isMe(member) && oldVoiceChannel) {
    ...
    const storedQueueChannels = await QueueChannelTable.getFromTarget(
      oldVoiceChannel.id
    );
    ...
    if (storedQueueChannel && storedQueueChannel.auto_fill) {
      const queueChannel = member.guild.channels.cache.get(
        storedQueueChannel.queue_channel_id
      ) as VoiceChannel;
      if (queueChannel) {
        await fillTargetChannel(
          storedQueueChannel,
...
export async function fillTargetChannel(
  ...
  if (dstChannel.permissionsFor(me).has("CONNECT")) {
    ...
    let storedQueueMembers = await QueueMemberTable.getFromQueue(
      srcChannel,
      "created_at"
    );
    if (storedQueueMembers.length > 0) {
      ...
      if (dstChannel.userLimit) {
        storedQueueMembers = storedQueueMembers.slice(
          0,
          dstChannel.userLimit -
            dstChannel.members.filter((member) => !member.user.bot).size
        );
      }
    ...
    const storedDisplayChannel = await DisplayChannelTable.getFromQueue(
      srcChannel.id
    ).first();
    if (storedDisplayChannel) {
      const displayChannel = me.guild.channels.cache.get(
...
async function reactionToggle(
  ...
  reaction = reaction.message.reactions.cache.find(
    (r) => r.emoji.name === config.joinEmoji
  ); // Handles a library bug
  ...
  const storedQueueMember = await QueueMemberTable.get(
    storedDisplayChannel.queue_channel_id,
    user.id
  );
  ...
  const queueChannel = reaction.message.guild.channels.cache.get(
    storedDisplayChannel.queue_channel_id
...
async function checkPatchNotes() {
  ...
  exists("../patch_notes/patch_notes.json", async (exists) => {
    if (!exists) return;
    // Collect notes
    const patchNotes: PatchNote[] = JSON.parse(
      readFileSync("../patch_notes/patch_notes.json", "utf8")
    );
    // Collect channel destinations and prefix
    for (const guild of client.guilds.cache.array()) {
      try {
        const prefix = (await QueueGuildTable.get(guild.id))?.prefix;
        const queueChannelId = (await QueueChannelTable.getFromGuild(guild))[0]
          ?.id;
        if (!queueChannelId) continue;
        const displayChannelId = (
          await DisplayChannelTable.get(queueChannelId).first()
        )?.display_channel_id;
        if (!displayChannelId) continue;
        const displayChannel = guild.channels.cache.get(displayChannelId) as
          | TextChannel
          | NewsChannel;
    ...
    for (const patchNote of patchNotes) {
      if (!patchNote.sent) {
        for (const [displayChannel, prefix] of displayChannels) {
          const parsedPatchNote: PatchNote = JSON.parse(
            JSON.stringify(patchNote)
          );
          parsedPatchNote.message.embed.fields.forEach((field) => {
            field.value = (field.value as string).replaceAll(
              config.prefix,
              prefix
            );
          });
    ...
    writeFileSync(
      ...
      JSON.stringify(patchNotes, null, 3)
    );
  });
}
